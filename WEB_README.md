...
- Files inside a tar (`.fwl2`, `.db2`) are read as world metadata: the world name, version, seed and player *count* from `.fwl2`, plus progression flags from `.db2`. Steam ids and character names from the player list are never retained or emitted, and a metadata file that cannot be parsed is reported as a per-archive error rather than failing the scan.
